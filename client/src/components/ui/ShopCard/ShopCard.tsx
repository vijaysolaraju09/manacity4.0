...
const ShopCard = ({ shop, onClick }: ShopCardProps) => (
  ...
  >
    ...
    <div className={styles.info}>
      <h3>{shop.name}</h3>
      <p className={cn('chip chip--brand', styles.category)}>{shop.category}</p>
      {shop.location && (
      ...
      <div className={styles.meta}>
        {shop.rating !== undefined && (
          <span className={cn('chip chip--success', styles.rating)}>
            ★ {shop.rating.toFixed(1)}
          </span>
        )}
        {shop.distance !== undefined && (
          <span className={cn('chip', styles.distance)}>{shop.distance} km</span>
        )}
        {shop.isOpen !== undefined && (
          <span
            className={cn(
              'chip',
              shop.isOpen ? 'chip--success' : 'chip--danger',
              styles.status,
            )}
          >
      ...
      <button
        type="button"
        className={cn('btn btn--brand btn--md', styles.cta)}
        onClick={(event) => {
