...
import { Filter, Loader2, MessageSquare, Star, X } from 'lucide-react';
...
const HistoryPage = () => {
  ...
  const filteredEntries = useMemo(() => {
    ...
    const end = normalizeDate(toDate, true);
    ...
    <main className={cn(styles.pageShell, 'bg-transparent text-slate-900 dark:text-slate-100')}>
      <div className={cn(styles.pageShell__inner, 'mx-auto max-w-5xl px-4 pb-20 pt-10 sm:px-6')}>
        ...
        {error ? (
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-3 rounded-2xl border border-slate-200/70 bg-white/80 p-4 shadow-sm shadow-slate-200/50 dark:border-slate-800/70 dark:bg-slate-900/60">
              <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.3em] text-indigo-500">
                <Filter className="h-4 w-4" aria-hidden="true" /> Filters
              </div>
              <div className="grid gap-3 md:grid-cols-[180px_1fr_1fr_120px] md:items-end">
            ...
            {filteredEntries.length === 0 ? (
              <Card className="rounded-3xl border border-slate-200/70 bg-white/85 p-6 text-center text-sm text-slate-600 shadow-xl shadow-slate-200/60 dark:border-slate-800/70 dark:bg-slate-900/70 dark:text-slate-200">
            ...
            {filteredEntries.map((entry) => {
              ...
              const isCompleted = entry.status.toLowerCase().includes('completed');
              const canShowFeedback = entry.canFeedback || isCompleted;
              return (
                <Card
                  key={entry.id}
